...
def build_room_scene_xml(walls, humans, out_path="mjx_room_scene.xml",
    ...
    # Humans are MOCAP bodies, walked kinematically by writing
    # data.mocap_pos each step.
    #
    # Their geoms do NOT collide (contype/conaffinity 0). A mocap body is
    # kinematic and therefore effectively infinite-mass, so any contact
    # imparts an unbounded impulse to the car. A trajectory trace caught
    # exactly that: the car launched to 4.9-7.6 m/s (calibrated top speed
    # ~3.5-3.9) and in one episode shoved clean through the perimeter to
    # y=+9.78 in a room whose wall sits at y=+8.0. Driving full-throttle
    # past moving people reached 43 m/s and |y|=215. That accounted for
    # the ~25/30 wall hits and near-zero success -- the car was being
    # thrown around, not failing to steer.
    #
    # People remain full obstacles where it matters: the lidar sees them
    # and the reward's safety term penalises approaching them, so
    # avoidance is still learned -- a pedestrian just can no longer
    # catapult the car.
    #
    ...
    for i, (hx_, hy_, hr) in enumerate(humans):
        ...
        geoms += (
            ...
            # Without an explicit mass MuJoCo infers one from the
            # default density of 1000 kg/m^3, giving each person ~326kg
            # and a total model mass of 981kg against a 3.5kg car. Inert
            # here because mocap bodies are kinematic, but it makes any
            # mass or inertia readout of the model nonsense.
            f'mass="70" '
